...
# 双向链表结点类
class BiNode(Node):
    def __init__(self, value: int, prior = None, next = None):
...
class LinkedList:
    ...
    # 使用列表类型的data创建链表
    def init_list(self, data: List[int]):
    ...
    # 获取链表的第i个元素
    def get_elem(self, index: int) -> int:
        p = self.get_node(index)
        return p.value

    # 修改链表的第i个元素的值
    def set_elem(self, index: int, value: int) -> None:
        p = self.get_node(index)
        p.value = value

    # 查询链表中是否存在指定值
    def in_list(self, value: int) -> bool:
        p = self.head
...
# 带尾指针的循环单链表类
class CirLinkedList(LinkedList):
